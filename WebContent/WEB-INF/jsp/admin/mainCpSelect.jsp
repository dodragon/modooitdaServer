...
function save(){
	...
	if(!title){
		...
		var nameArr = new Array('cpSeq', 'title', 'menuSeq', 'imgUrl');
		var valueArr = new Array(cpSeq, title, menuSeq, imgUrl);
		
		var form = document.createElement("form");
		form.setAttribute("charset", "UTF-8");
		form.setAttribute("method", "Post"); // Get 또는 Post 입력
		form.setAttribute("action", "./insertMainList.api");
...
<nav class="navbar navbar-expand navbar-dark bg-dark static-top">
      <a class="navbar-brand mr-1" href="./mainpage.api">메인 리스트 설정</a>
	      ...
	      <div class="card mb-3">
	         <div class="card-header">
	            ...
	            <c:forEach items="${menuList}" var="menu">
	            	...
	            	<label for="${menu.seqNum }">${menu.menuName }  (<fmt:formatNumber value="${menu.price }" pattern="###,###원"/> => <fmt:formatNumber value="${menu.disPrice }" pattern="###,###원"/>, ${menu.percentAge }%)</label><br>
	            </c:forEach>
	            <br>
	            <hr>
	            ...
	            <div class="img_container">
		            <c:forEach items="${imgList}" var="img">
		            	<div class="img_item">
		            		...
		            		<label for="${img.seq_num }_img"><img onclick="imgClick(this)" class="imgs" src="${img.img_url }" /></label>
		            	</div>
		            </c:forEach>
	            </div>
	            <br><hr><br>
