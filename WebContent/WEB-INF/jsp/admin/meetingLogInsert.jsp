...
function dateSetting(today){
	...
	document.getElementById('meetDate').value = year + '-' + month + '-' + date;
}
...
function insert(){
	...
	if(!meetTitle){
		...
		var nameArr = new Array('meetTitle', 'mainAgenda', 'place', 'meetDate', 'startTime', 'endTime', 'contents', 'result', 'director', 'writer', 'attendees');
		var valueArr = new Array(meetTitle, mainAgenda, place, meetDate, startTime, endTime, contents, result, director, writer, attendees);
		
		var form = document.createElement("form");
		form.setAttribute("charset", "UTF-8");
		form.setAttribute("method", "Post"); // Get 또는 Post 입력
		form.setAttribute("action", "./meetLogInsertSuccess.api");
          ...
          <div class="card mb-3">
            <div class="card-header">
              <div id="default-info">
              	<h3>회의록</h3>
              	<div id="form-div">
	              	<input id="meet_title" type="text" placeholder="회의 제목을 작성해 주세요." maxlength="200"/><br>
	              	<div id="agenda_div">
	              		<input id="main_agenda" type="text" placeholder="주요 안건" />
	              		<input id="place" type="text" placeholder="회의 장소" />
	              	</div>
	              	...
	              	<textarea id="contents" style="resize: none;" placeholder="회의 내용을 작성해 주세요."></textarea>
	              	<textarea id="result" style="resize: none;" placeholder="회의 결과룰 작성해 주세요."></textarea><br><br>
	              	<div id="person_div">
	              		<div>
		              		책임자<br>
			              	<select id="director">
			              		<c:forEach items="${adminList }" var="mem">
			              			<option>${mem.job_class } 님(${mem.department })</option>
			              		</c:forEach>
		              		</select>
	              		</div>
	              	</div><br>
	              	<div>
	              		참석자<br>
	              		<c:forEach items="${adminList }" var="mem">
	              			<input class="attendees" id="${mem.name }" type="checkbox" value="${mem.job_class } 님(${mem.department })" />
	              			<label for="${mem.name }">${mem.job_class } 님(${mem.department })</label><br>
	              		</c:forEach>
	              	</div>
              	</div>
              </div>
