...
window.onload = function(){
	...
	$('#status-selector').val("${vo.inspection}").attr('selected', 'selected');
}

function updateSubmit(){
	var divCode = '<%=session.getAttribute("divCode") %>';
...
<nav class="navbar navbar-expand navbar-dark bg-dark static-top">
      <a class="navbar-brand mr-1" href="./mainpage.api">앱 정보 수정</a>
      ...
      <ul class="sidebar navbar-nav">
        ...
        <li class="nav-item dropdown">
          ...
          <div class="dropdown-menu" aria-labelledby="pagesDropdown">
            ...
            <a class="dropdown-item" href="#">앱 정보 수정</a>
            <div class="dropdown-divider"></div>
        ...
        <div class="container-fluid">
          ...
          <div class="card mb-3">
            <div class="card-header">
              <div id="default-info">
              	<form id="form" action="./updateAppInfo.api" method="post">
              		<p>앱 버전</p>
              		<input name="version" type="number" value="${vo.new_version }"><br>
              		<p class="title-p">앱 상태</p>
              		<select name="status" id="status-selector">
              			<option>정상</option>
              			<option>점검</option>
              		</select>
              		<p class="title-p">앱 관리자 계정 (${vo.almightyID })</p>
              		<input name="pw" type="text" placeholder="변경할 비밀번호" value="${vo.almightyPW }">
              	</form>
              	<button id="submit-btn" onclick="updateSubmit()">수정</button>
              </div>
            </div>
            </div>
          </div>
